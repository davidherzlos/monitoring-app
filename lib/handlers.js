...
handlers._users.put = (data, callback) => {
    ...
    if (phone) {
        ...
        if (firstName || lastName || password) {
            ...
            callback(400, {'Error': 'Missins fields to update'})
        }
    } else {
                ...
                _data.read('users', phone, (err, userData) => {
                    if (!err && userData) {
                        _data.delete('users', phone, err => {
                            if (!err) {
                                // Delete all checks assoctated with the user.
                                const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : []
                                const checksToDelete = userChecks.length
                                if (checksToDelete > 0) {
                                    var checksDeleted = 0
                                    var deletionErrors = false
                                    // Look trhough the checks.
                                        ...
                                        _data.delete('checks', checkId, err => {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    callback(500, {'Error': 'Errors encountered while attempting to delete all of the usets checks. All checks may not have been deleted from the system successfuly'})
                                                }
                                            }
                                        })
                                    });
        ...
        _data.read('tokens', id, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    // Set the expiration an hout from now.
...
// Verify if the given token id is vcurrently valid for a given user.
        ...
        _data.read('tokens', token, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : []
                        // Verify that the user has less than the number os max-checks-per-user.
...
// Checks - get
// Required data: idchecks:create
// Optional data: none
handlers._checks.get = (data, callback) => {
    handlers._checks._get_with_promises(data)
        .then((checkData) => {
            callback(200, checkData)
        }).catch((error) => {
            callback(error.code, {'Error': error.reason})
        })
...
handlers._checks._get_with_promises = (data) => {
    return new Promise((resolve, reject) => {
        ...
        if (typeof(id) !== 'string' || id.trim().length !== 20) {
            return reject({code: 400, reason: 'Missing required field'})
        }
        
        // Lookup the check.
        _data.read('checks', id, (err, checkData) => {
            if (err) {
                return reject({code: 404, reason: 'The check was not found'})
            }
            // Get the token from the headers.
            const token = data.headers.token
            if (!token || typeof(token) !== 'string') {
                return reject({code: 403, reason: 'Token is required'})
            }
            // Verify that the given token is valid and belongs to the user who created the check.
            const phone = checkData.userPhone
            handlers._tokens.verifyToken(token, phone, isValid => {
                if (!isValid) {
                    return reject({code: 403, reason: 'Token is not valid'})
                }
                // Return the check data.
                return resolve(checkData)
            })
        })
    })
}
